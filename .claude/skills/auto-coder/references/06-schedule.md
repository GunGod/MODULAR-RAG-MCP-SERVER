...
|---------|---------|------|---------|------|
| A1 | 初始化目录树与最小可运行入口 | [x] | 2025-03-14 | 已完成项目目录树创建、main.py 入口、pyproject.toml 配置 |
| A2 | 引入 pytest 并建立测试目录约定 | [x] | 2026-03-14 | 已完成 pytest 配置与冒烟测试 |
| A3 | 配置加载与校验（Settings） | [x] | 2026-03-14 | 已完成 Settings 数据类、加载器、校验逻辑与测试 |

#### 阶段 B：Libs 可插拔层

| 任务编号 | 任务名称 | 状态 | 完成日期 | 备注 |
|---------|---------|------|---------|------|
| B1 | LLM 抽象接口与工厂 | [x] | 2026-03-14 | 已完成 BaseLLM 接口、LLMFactory、FakeLLM 测试实现 |
| B2 | Embedding 抽象接口与工厂 | [x] | 2026-03-14 | 已完成 BaseEmbedding 接口、EmbeddingFactory、FakeEmbedding 测试实现 |
| B3 | Splitter 抽象接口与工厂 | [x] | 2026-03-17 | 已完成 BaseSplitter 接口、SplitterFactory、FakeSplitter 测试实现 |
| B4 | VectorStore 抽象接口与工厂 | [x] | 2026-03-17 | 已完成 BaseVectorStore 接口、VectorStoreFactory、FakeVectorStore 测试实现 |
| B5 | Reranker 抽象接口与工厂（含 None 回退） | [x] | 2026-03-17 | 已完成 BaseReranker 接口、RerankerFactory、NoneReranker 测试实现 |
| B6 | Evaluator 抽象接口与工厂 | [x] | 2026-03-17 | 已完成 BaseEvaluator 接口、EvaluatorFactory、CustomEvaluator 测试实现 |
...
| B9 | Azure Vision LLM 实现 | [x] | 2026-03-21 | 已完成 AzureVisionLLM 实现、Azure OpenAI Vision API 调用、支持 URL/base64/PATH 三种图片输入、自动图片压缩、29个测试用例全部通过 |
...
| C3 | Loader 抽象基类与 PDF Loader | [x] | 2026-03-22 | 已完成 BaseLoader 抽象基类、PdfLoader 实现（MarkItDown + PyPDF2 图片提取）、图片占位符插入、降级行为、16个测试用例全部通过 |
